...
var nobleCurves = (() => {
  ...
  function tonelliShanks(P3) {
    ...
    return function tonelliSlow(Fp9, n) {
      ...
      while (!Fp9.eql(b, Fp9.ONE)) {
        ...
        const ge = Fp9.pow(g, _1n2 << BigInt(r - m - 1));
        g = Fp9.sqr(ge);
        x = Fp9.mul(x, ge);
        b = Fp9.mul(b, g);
        r = m;
  ...
  function FpSqrt(P3) {
    ...
    if (P3 % _8n === _5n) {
      ...
      return function sqrt5mod8(Fp9, n) {
        const n2 = Fp9.mul(n, _2n2);
        const v = Fp9.pow(n2, c1);
        const nv = Fp9.mul(n, v);
        const i = Fp9.mul(Fp9.mul(nv, _2n2), v);
  ...
  function FpSqrtOdd(Fp9, elm) {
    if (!Fp9.isOdd)
      throw new Error(`Field doesn't have isOdd`);
    const root = Fp9.sqrt(elm);
    return Fp9.isOdd(root) ? root : Fp9.neg(root);
  }
  ...
  function wNAF(c, bits) {
    ...
    return {
      ...
      wNAFCached(P3, n, transform) {
        const W = pointWindowSizes.get(P3) || 1;
  ...
  function weierstrassPoints(opts) {
    ...
    const toBytes2 = CURVE2.toBytes || ((_c, point, _isCompressed) => {
      ...
      return concatBytes(Uint8Array.from([4]), Fp9.toBytes(a.x), Fp9.toBytes(a.y));
    });
    ...
    if (!Fp9.eql(Fp9.sqr(CURVE2.Gy), weierstrassEquation(CURVE2.Gx)))
    ...
    const assertValidMemo = memoized((p) => {
      if (p.is0()) {
        if (CURVE2.allowInfinityPoint && !Fp9.is0(p.py))
          return;
    ...
    class Point2 {
       ...
       */
      negate() {
        return new Point2(this.px, Fp9.neg(this.py), this.pz);
      }
      ...
      add(other) {
        ...
        let X3 = Fp9.ZERO, Y3 = Fp9.ZERO, Z3 = Fp9.ZERO;
        const a = CURVE2.a;
        const b3 = Fp9.mul(CURVE2.b, _3n2);
       ...
       */
      multiplyUnsafe(sc) {
        aInRange("scalar", sc, _0n4, CURVE2.n);
        const I = Point2.ZERO;
        if (sc === _0n4)
          return I;
        if (sc === _1n4)
        ...
        if (k2neg)
          k2p = k2p.negate();
        k2p = new Point2(Fp9.mul(k2p.px, endo.beta), k2p.py, k2p.pz);
        return k1p.add(k2p);
      }
      /**
    ...
    Point2.BASE = new Point2(CURVE2.Gx, CURVE2.Gy, Fp9.ONE);
    Point2.ZERO = new Point2(Fp9.ZERO, Fp9.ONE, Fp9.ZERO);
  ...
  function weierstrass(curveDef) {
    ...
    const { Fp: Fp9, n: CURVE_ORDER } = CURVE2;
    ...
    function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
      ...
      if (ent != null && ent !== false) {
        const e = ent === true ? randomBytes2(Fp9.BYTES) : ent;
  ...
  function SWUFpSqrtRatio(Fp9, Z) {
    ...
    const c7 = Fp9.pow(Z, (c2 + _1n4) / _2n3);
    let sqrtRatio = (u, v) => {
      let tv1 = c6;
    ...
    if (Fp9.ORDER % _4n2 === _3n2) {
      const c12 = (Fp9.ORDER - _3n2) / _4n2;
  ...
  function twistedEdwards(curveDef) {
    ...
    const { Fp: Fp9, n: CURVE_ORDER, prehash, hash: cHash, randomBytes: randomBytes2, nByteLength, h: cofactor } = CURVE2;
    const MASK = _2n5 << BigInt(nByteLength * 8) - _1n6;
    const modP2 = Fp9.create;
    ...
    class Point2 {
      ...
      static fromHex(hex, zip215 = false) {
        ...
        const max = zip215 ? MASK : Fp9.ORDER;
      ...
      toRawBytes() {
        ...
        const bytes2 = numberToBytesLE(y, Fp9.BYTES);
    ...
    function sign(msg, privKey, options = {}) {
      ...
      const res = concatBytes(R, numberToBytesLE(s, Fp9.BYTES));
    ...
    const utils2 = {
      ...
      randomPrivateKey: () => randomBytes2(Fp9.BYTES),
      /**
  ...
  function edwardsToMontgomeryPub(edwardsPub) {
    ...
    return Fp2.toBytes(Fp2.create((_1n15 + y) * Fp2.inv(_1n15 - y)));
  }
  ...
  function edwardsToMontgomeryPub2(edwardsPub) {
    ...
    const _1n15 = BigInt(1);
    return Fp3.toBytes(Fp3.create((y - _1n15) * Fp3.inv(y + _1n15)));
  }
  ...
  function bls(CURVE2) {
    const { Fp: Fp9, Fr: Fr3, Fp2: Fp24, Fp6: Fp64, Fp12: Fp123 } = CURVE2.fields;
  ...
  var _2n10 = BigInt(2);
  var _3n6 = BigInt(3);
  function calcFrobeniusCoefficients(Fp9, nonResidue, modulus, degree, num2 = 1, divisor) {
    ...
    for (let i = 0; i < num2; i++) {
      ...
      for (let j = 0, qPower = _1n12; j < degree; j++) {
        ...
        powers.push(Fp9.pow(nonResidue, power));
        qPower *= modulus;
      }
      res.push(powers);
    }
  ...
  function psiFrobenius(Fp9, Fp24, base) {
    const PSI_X = Fp24.pow(base, (Fp9.ORDER - _1n12) / _3n6);
    const PSI_Y = Fp24.pow(base, (Fp9.ORDER - _1n12) / _2n10);
    function psi2(x, y) {
      const x2 = Fp24.mul(Fp24.frobeniusMap(x, 1), PSI_X);
      const y2 = Fp24.mul(Fp24.frobeniusMap(y, 1), PSI_Y);
      return [x2, y2];
    }
    const PSI2_X = Fp24.pow(base, (Fp9.ORDER ** _2n10 - _1n12) / _3n6);
    const PSI2_Y = Fp24.pow(base, (Fp9.ORDER ** _2n10 - _1n12) / _2n10);
  ...
  function tower12(opts) {
    const { ORDER } = opts;
    const Fp9 = Field(ORDER);
    const FpNONRESIDUE = Fp9.create(opts.NONRESIDUE || BigInt(-1));
    const FpLegendre2 = FpLegendre(ORDER);
    const Fpdiv2 = Fp9.div(Fp9.ONE, _2n10);
    const FP2_FROBENIUS_COEFFICIENTS = calcFrobeniusCoefficients(Fp9, FpNONRESIDUE, Fp9.ORDER, 2)[0];
    ...
    const Fp2Multiply = ({ c0, c1 }, rhs) => {
      if (typeof rhs === "bigint")
        return { c0: Fp9.mul(c0, rhs), c1: Fp9.mul(c1, rhs) };
      const { c0: r0, c1: r1 } = rhs;
      let t1 = Fp9.mul(c0, r0);
      let t2 = Fp9.mul(c1, r1);
      const o0 = Fp9.sub(t1, t2);
      const o1 = Fp9.sub(Fp9.mul(Fp9.add(c0, c1), Fp9.add(r0, r1)), Fp9.add(t1, t2));
      return { c0: o0, c1: o1 };
    };
    const Fp2Square = ({ c0, c1 }) => {
      const a = Fp9.add(c0, c1);
      const b = Fp9.sub(c0, c1);
      const c = Fp9.add(c0, c0);
      return { c0: Fp9.mul(a, b), c1: Fp9.mul(c, c1) };
    };
    const Fp2fromBigTuple = (tuple) => {
      if (tuple.length !== 2)
        throw new Error("Invalid tuple");
      const fps = tuple.map((n) => Fp9.create(n));
    ...
    const Fp24 = {
      ...
      ZERO: { c0: Fp9.ZERO, c1: Fp9.ZERO },
      ONE: { c0: Fp9.ONE, c1: Fp9.ZERO },
      create: (num2) => num2,
      isValid: ({ c0, c1 }) => typeof c0 === "bigint" && typeof c1 === "bigint",
      is0: ({ c0, c1 }) => Fp9.is0(c0) && Fp9.is0(c1),
      eql: ({ c0, c1 }, { c0: r0, c1: r1 }) => Fp9.eql(c0, r0) && Fp9.eql(c1, r1),
      neg: ({ c0, c1 }) => ({ c0: Fp9.neg(c0), c1: Fp9.neg(c1) }),
      ...
      div: (lhs, rhs) => Fp24.mul(lhs, typeof rhs === "bigint" ? Fp9.inv(Fp9.create(rhs)) : Fp24.inv(rhs)),
      inv: ({ c0: a, c1: b }) => {
        const factor = Fp9.inv(Fp9.create(a * a + b * b));
        return { c0: Fp9.mul(factor, Fp9.create(a)), c1: Fp9.mul(factor, Fp9.create(-b)) };
      },
      sqrt: (num2) => {
        if (opts.Fp2sqrt)
        ...
        const { c0, c1 } = num2;
        if (Fp9.is0(c1)) {
          if (Fp9.eql(FpLegendre2(Fp9, c0), Fp9.ONE))
            return Fp24.create({ c0: Fp9.sqrt(c0), c1: Fp9.ZERO });
          else
            return Fp24.create({ c0: Fp9.ZERO, c1: Fp9.sqrt(Fp9.div(c0, FpNONRESIDUE)) });
        }
        const a = Fp9.sqrt(Fp9.sub(Fp9.sqr(c0), Fp9.mul(Fp9.sqr(c1), FpNONRESIDUE)));
        let d = Fp9.mul(Fp9.add(a, c0), Fpdiv2);
        const legendre = FpLegendre2(Fp9, d);
        if (!Fp9.is0(legendre) && !Fp9.eql(legendre, Fp9.ONE))
          d = Fp9.sub(d, a);
        const a0 = Fp9.sqrt(d);
        const candidateSqrt = Fp24.create({ c0: a0, c1: Fp9.div(Fp9.mul(c1, Fpdiv2), a0) });
      ...
      fromBytes(b) {
        ...
        return { c0: Fp9.fromBytes(b.subarray(0, Fp9.BYTES)), c1: Fp9.fromBytes(b.subarray(Fp9.BYTES)) };
      },
      toBytes: ({ c0, c1 }) => concatBytes(Fp9.toBytes(c0), Fp9.toBytes(c1)),
      ...
      frobeniusMap: ({ c0, c1 }, power) => ({
        c0,
        c1: Fp9.mul(c1, FP2_FROBENIUS_COEFFICIENTS[power % 2])
      })
    };
    ...
    const [FP6_FROBENIUS_COEFFICIENTS_1, FP6_FROBENIUS_COEFFICIENTS_2] = calcFrobeniusCoefficients(Fp24, Fp2Nonresidue, Fp9.ORDER, 6, 2, 3);
    const Fp64 = {
      ORDER: Fp24.ORDER,
      ...
      div: (lhs, rhs) => Fp64.mul(lhs, typeof rhs === "bigint" ? Fp9.inv(Fp9.create(rhs)) : Fp64.inv(rhs)),
      pow: (num2, power) => FpPow(Fp64, num2, power),
      invertBatch: (nums) => FpInvertBatch(Fp64, nums),
      // Normalized
      add: Fp6Add,
      sub: Fp6Subtract,
      mul: Fp6Multiply,
    ...
    const FP12_FROBENIUS_COEFFICIENTS = calcFrobeniusCoefficients(Fp24, Fp2Nonresidue, Fp9.ORDER, 12, 1, 6)[0];
    ...
    const Fp12Multiply = ({ c0, c1 }, rhs) => {
      if (typeof rhs === "bigint")
        return { c0: Fp64.mul(c0, rhs), c1: Fp64.mul(c1, rhs) };
      let { c0: r0, c1: r1 } = rhs;
      let t1 = Fp64.mul(c0, r0);
      let t2 = Fp64.mul(c1, r1);
      return {
        c0: Fp64.add(t1, Fp64.mulByNonresidue(t2)),
        // T1 + T2 * v
        // (c0 + c1) * (r0 + r1) - (T1 + T2)
        c1: Fp64.sub(Fp64.mul(Fp64.add(c0, c1), Fp64.add(r0, r1)), Fp64.add(t1, t2))
      };
    };
    const Fp12Square = ({ c0, c1 }) => {
      let ab = Fp64.mul(c0, c1);
      return {
        // (c1 * v + c0) * (c0 + c1) - AB - AB * v
        c0: Fp64.sub(Fp64.sub(Fp64.mul(Fp64.add(Fp64.mulByNonresidue(c1), c0), Fp64.add(c0, c1)), ab), Fp64.mulByNonresidue(ab)),
        c1: Fp64.add(ab, ab)
      };
    };
    ...
    const Fp123 = {
      ...
      ZERO: { c0: Fp64.ZERO, c1: Fp64.ZERO },
      ONE: { c0: Fp64.ONE, c1: Fp64.ZERO },
      create: (num2) => num2,
      isValid: ({ c0, c1 }) => Fp64.isValid(c0) && Fp64.isValid(c1),
      is0: ({ c0, c1 }) => Fp64.is0(c0) && Fp64.is0(c1),
      neg: ({ c0, c1 }) => ({ c0: Fp64.neg(c0), c1: Fp64.neg(c1) }),
      eql: ({ c0, c1 }, { c0: r0, c1: r1 }) => Fp64.eql(c0, r0) && Fp64.eql(c1, r1),
      sqrt: notImplemented,
      inv: ({ c0, c1 }) => {
        let t = Fp64.inv(Fp64.sub(Fp64.sqr(c0), Fp64.mulByNonresidue(Fp64.sqr(c1))));
        return { c0: Fp64.mul(c0, t), c1: Fp64.neg(Fp64.mul(c1, t)) };
      },
      div: (lhs, rhs) => Fp123.mul(lhs, typeof rhs === "bigint" ? Fp9.inv(Fp9.create(rhs)) : Fp123.inv(rhs)),
      ...
      fromBytes: (b) => {
        if (b.length !== Fp123.BYTES)
          throw new Error(`fromBytes wrong length=${b.length}`);
        return {
          c0: Fp64.fromBytes(b.subarray(0, Fp64.BYTES)),
          c1: Fp64.fromBytes(b.subarray(Fp64.BYTES))
        };
      },
      toBytes: ({ c0, c1 }) => concatBytes(Fp64.toBytes(c0), Fp64.toBytes(c1)),
      cmov: ({ c0, c1 }, { c0: r0, c1: r1 }, c) => ({
        c0: Fp64.cmov(c0, r0, c),
        c1: Fp64.cmov(c1, r1, c)
      }),
      // Utils
      // toString() {
      ...
      // }
      fromBigTwelve: (t) => ({
        c0: Fp64.fromBigSix(t.slice(0, 6)),
        c1: Fp64.fromBigSix(t.slice(6, 12))
      }),
      // Raises to q**i -th power
      frobeniusMap(lhs, power) {
        const { c0, c1, c2 } = Fp64.frobeniusMap(lhs.c1, power);
        const coeff = FP12_FROBENIUS_COEFFICIENTS[power % 12];
        return {
          c0: Fp64.frobeniusMap(lhs.c0, power),
          c1: Fp64.create({
      ...
      mulByFp2: ({ c0, c1 }, rhs) => ({
        c0: Fp64.mulByFp2(c0, rhs),
        c1: Fp64.mulByFp2(c1, rhs)
      }),
      conjugate: ({ c0, c1 }) => ({ c0, c1: Fp64.neg(c1) }),
      // Sparse multiplication
      mul014: ({ c0, c1 }, o0, o1, o4) => {
        let t0 = Fp64.mul01(c0, o0, o1);
        let t1 = Fp64.mul1(c1, o4);
        return {
          c0: Fp64.add(Fp64.mulByNonresidue(t1), t0),
          // T1 * v + T0
          // (c1 + c0) * [o0, o1+o4] - T0 - T1
          c1: Fp64.sub(Fp64.sub(Fp64.mul01(Fp64.add(c1, c0), o0, Fp24.add(o1, o4)), t0), t1)
        };
      },
      mul034: ({ c0, c1 }, o0, o3, o4) => {
        ...
        const b = Fp64.mul01(c1, o3, o4);
        const e = Fp64.mul01(Fp64.add(c0, c1), Fp24.add(o0, o3), o4);
        return {
          c0: Fp64.add(Fp64.mulByNonresidue(b), a),
          c1: Fp64.sub(e, Fp64.add(a, b))
        };
      },
    ...
    return { Fp: Fp9, Fp2: Fp24, Fp6: Fp64, Fp4Square: Fp4Square3, Fp12: Fp123 };
  }
  ...
  var { Fp: Fp7, Fp2: Fp22, Fp6: Fp62, Fp4Square, Fp12 } = tower12({
    // Order of Fp
    ...
    Fp2mulByB: ({ c0, c1 }) => {
      const t0 = Fp7.mul(c0, _4n4);
      const t1 = Fp7.mul(c1, _4n4);
  ...
  var G2_SWU = mapToCurveSimpleSWU(Fp22, {
    A: Fp22.create({ c0: Fp7.create(_0n12), c1: Fp7.create(BigInt(240)) }),
    // A' = 240 * I
  ...
  var { G2psi, G2psi2 } = psiFrobenius(Fp7, Fp22, Fp22.div(Fp22.ONE, Fp22.NONRESIDUE));
  ...
  var COMPRESSED_ZERO = setMask(Fp7.toBytes(_0n12), { infinity: true, compressed: true });
  ...
  function signatureG2ToRawBytes(point) {
    ...
    const sort = Boolean(tmp / Fp7.ORDER & _1n13);
    const z2 = x0;
    ...
    G1: {
      ...
      fromBytes: (bytes2) => {
        ...
        if (value.length === 48 && compressed) {
          ...
          const right = Fp7.add(Fp7.pow(x, _3n7), Fp7.create(bls12_381.params.G1b));
  ...
  var { Fp: Fp8, Fp2: Fp23, Fp6: Fp63, Fp4Square: Fp4Square2, Fp12: Fp122 } = tower12({
  ...
  var { G2psi: G2psi3, psi } = psiFrobenius(Fp8, Fp23, Fp23.NONRESIDUE);
  ...
  var bn254 = bls({
    // Fields
    fields: { Fp: Fp8, Fp2: Fp23, Fp6: Fp63, Fp12: Fp122, Fr: Fr2 },
    G1: {
      Fp: Fp8,
      h: BigInt(1),
      Gx: BigInt(1),
